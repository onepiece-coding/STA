...
import { validateCreateSeller, validateCreateDelivery } from '../validations/userValidations.js';
...
router.route('/sellers/:id')
      ...
      .patch(authorizeRoles('admin', 'seller'), updateSellerCtrl)
...
router.route('/delivery/:id')
      .all(validateObjectIdParam('id'))
      .get(authorizeRoles('admin', 'seller'), getDeliveryByIdCtrl) 
