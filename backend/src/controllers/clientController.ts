 ...
 */
export const createClientCtrl = asyncHandler(async (req: MulterReq, res: Response) => {
  const { name, location, typeOfBusiness, city, sector, phoneNumber, deliveryMan } = req.body;
  ...
  if (!seller.sectors?.map(String).includes(sector)) {
    throw createError(403, 'Cannot create client outside your assigned sectors');
  }
  ...
  if (await Client.exists({ name, seller: sellerId })) {
  ...
  if (req.file) {
    const uploadRes = await cloudinaryUploadImage(req.file.buffer, { folder: 'clients' });
    data.placePicUrl = {
 ...
 */
export const getClientsCtrl = asyncHandler(async (req: Request, res: Response) => {
  const user = req.user!;  
  const { sectorId, cityId } = req.query as { sectorId?: string; cityId?: string };
 ...
 * @access private(seller/delivery)
 */
export const getClientByIdCtrl = asyncHandler(async (req: Request, res: Response) => {
  const user = req.user!;
 ...
 * @access private(seller)
 */
export const updateClientCtrl = asyncHandler(async (req: MulterReq, res: Response) => {
  const client = await Client.findOne({ _id: req.params.id, seller: req.user!._id });
  if (!client) throw createError(404, 'Client not found');

  // Update fields
  ['name','location','typeOfBusiness', 'city', 'sector', 'phoneNumber', 'deliveryMan'].forEach(f => {
  ...
  if (req.file) {
    ...
    const up = await cloudinaryUploadImage(req.file.buffer, { folder: 'clients' });
 ...
 * @access private(seller)
 */
export const deleteClientCtrl = asyncHandler(async (req: Request, res: Response) => {
  const client = await Client.findOneAndDelete({ _id: req.params.id, seller: req.user!._id });
  if (!client) throw createError(404, 'Client not found');
