...
---------------------------------------------------*/
export const createSectorCtrl = asyncHandler(
...
---------------------------------------------------*/
export const getSectorsByCityCtrl = asyncHandler(
  async (req: Request, res: Response) => {
    const { cityId } = req.params;
    const sectors = await Sector.find({ city: cityId }).populate(
      'city',
      'name',
    );
 ...
 * @access private (admin only)
---------------------------------------------------*/
export const deleteSectorCtrl = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    res
      .status(200)
      .json({ message: 'Sector deleted and removed from all users' });
  },
);
