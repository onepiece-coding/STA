...
export const validateCreateOrder = [
  ...
  body('items.*.quantity')
    .isInt({gt:0}).withMessage('quantity must be >0'),
...
export const validateUpdateOrder = [
  ...
  body('items.*.productId')
    .optional()
    .isMongoId().withMessage('items[].productId must be a valid MongoId'),
  body('items.*.soldBy')
    .optional()
    .isIn(['unit','carton']).withMessage('items[].soldBy must be "unit" or "carton"'),
  body('items.*.quantity')
    .optional()
    .isInt({ gt: 0 }).withMessage('items[].quantity must be a positive integer'),

  // wantedDate only if delivery
  body('wantedDate')
    .optional()
    .isISO8601().withMessage('wantedDate must be a valid date'),
...
export const validateGetOrders = [
  ...
  query('status').optional().isIn(['pending','inProgress','done','cancelled']),
