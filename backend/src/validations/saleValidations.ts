...
export const validateCreateSale: Array<ValidationChain | RequestHandler> = [
  ...
  body('items.*.soldBy')
    .exists()
    .withMessage('Each item must have soldBy')
    .bail()
    .isIn(['unit', 'carton'])
    .withMessage('soldBy must be "unit" or "carton"'),
  body('items.*.quantity')
    .exists()
    .withMessage('Each item must have quantity')
    .bail()
    .isInt({ gt: 0 })
...
export const validateUpdateSale: Array<ValidationChain | RequestHandler> = [
  body('deliveryStatus')
    ...
    .withMessage(
      'deliveryStatus must be one of "ordered", "delivered", "notDelivered"',
    ),

  body('return.returnItems')
    .optional()
    .isArray()
    .withMessage('return.returnItems must be an array'),
  body('return.returnItems.*.productId')
    .optional()
    .isMongoId()
    .withMessage('return.returnItems[].productId must be a valid MongoId'),
  body('return.returnItems.*.soldBy')
    .optional()
    .isIn(['unit', 'carton'])
    .withMessage('return.returnItems[].soldBy must be "unit" or "carton"'),
  body('return.returnItems.*.quantity')
    .optional()
    .isInt({ gt: 0 })
    .withMessage('return.returnItems[].quantity must be a positive integer'),
  body('return.returnTotal')
    .optional()
    .isFloat({ min: 0 })
    .withMessage('return.returnTotal must be a non-negative number'),
  body('returnGlobal')
    .optional()
    .isFloat({ min: 0 })
    .withMessage('returnGlobal must be a non-negative number'),
  body('paymentType')
    .optional()
    .isIn(['espece', 'card', 'other'])
    .withMessage('paymentType must be "espece", "card", or "other"'),
  body('amountPaid')
    .optional()
    .isFloat({ min: 0 })
