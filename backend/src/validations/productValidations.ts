...
export const validateCreateProduct = [
  ...
  body('discountRule.minQty')
    .isInt({ min: 1 })
    .withMessage('discountRule.minQty must be at least 1'),
  body('discountRule.percent')
    .isFloat({ min: 0, max: 100 })
    .withMessage('discountRule.percent must be between 0 and 100'),
...
export const validateUpdateProduct = [
  ...
  body('globalDiscountPercent')
    .optional()
    .isFloat({ min: 0, max: 100 })
    .withMessage('globalDiscountPercent must be between 0 and 100'),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return next(
